...
float xu_li(float a, float b);
void tra_ve_du_lieu(float a, float b, float x);

// ax^2 + bx + c = 0
void nhap_lieu(float *a, float *b,float *c);
float tinh_toan(float delta, float x1, float x2);
void tra_ve(float x1, float x2);
 
int main() {
	int chon;
	printf("           Menu          \n");
	printf("    Nhap lua chon tinh:  \n");
	printf("    1 la xet tam giac    \n");
	printf("    2 la tinh ax+b=0     \n");
	printf("    3 la tinh ax^2 + bx +c =0     \n");
	scanf("%d", &chon);
	switch(chon){
		case 1:			
...
void kiem_tra_tam_giac(float AB, float AC, float BC){
	...
	int degA = (radA * 180.0) / M_PI;
	int degB = (radB * 180.0) / M_PI;
	int degC = (radC * 180.0) / M_PI;
	...
	if (AB+AC>BC && AC+BC>AB && BC+AB>AC){
		...
		} else if ((ceil(degA)==90 || ceil(degB)==90 || ceil(degC)==90) && (AB==AC || AC==BC || BC==AB)){
